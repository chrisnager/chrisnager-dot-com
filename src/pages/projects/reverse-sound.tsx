...
const SUPPORTED_AUDIO_EXTENSIONS = [`.mp3`, `.m4a`, `.aac`, `.wav`, `.webm`, `.ogg`]
...
export default function ReverseSound() {
    ...
    function isSupportedAudioFile(file: File) {
        ...
        const hasSupportedExtension = SUPPORTED_AUDIO_EXTENSIONS.some((extension) => lowerCaseName.endsWith(extension))

        return SUPPORTED_AUDIO_MIME_TYPES.has(file.type) || hasSupportedExtension
    }
    ...
    function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
        ...
        if (file.size > MAX_FILE_SIZE_BYTES) {
            setAudioFile(null)
            setErrorMessage(`Please choose an audio file smaller than 25 MB.`)
            return
        }

        if (!isSupportedAudioFile(file)) {
            setAudioFile(null)
            setErrorMessage(`Please choose a supported audio file: MP3, M4A, AAC, WAV, WebM, or Ogg.`)
            return
        }
    ...
    return (
        <Layout>
            <Box sx={{ maxWidth: `50ch`, marginBlockEnd: 5, paddingInline: 3 }}>
                ...
                <p><i>See a <a href="https://github.com/chrisnager/chrisnager-dot-com/pull/51" target="_blank" rel="noopener noreferrer">demo</a></i>.</p>
